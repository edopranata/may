        ...
        <div class="w-3xl print:w-[100%]">
            ...
            <div class="grid text-sm grid-cols-1 w-full">
                ...
                <div class="grid grid-cols-5" v-for="(item, index) in props.invoice.trades" :key="item.id">
                    ...
                    <span class="px-4 border-gray-800 border-l text-right">{{ Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.car_fee) }}</span>
                    <span class="px-4 border-gray-800 border-x text-right">{{ Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.net_weight * item.car_fee) }}</span>
                </div>
...
onMounted( () => {
    ...
    let total = props.invoice.trades.reduce((arr, trades) => {
        arr.push(trades.net_weight * trades.car_fee)
        return (arr)
    }, []);
