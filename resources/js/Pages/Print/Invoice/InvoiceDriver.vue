        ...
        <div class="w-3xl print:w-[100%]">
            ...
            <div class="grid text-sm grid-cols-1 w-full">
                <div class="flex justify-between border-gray-800 border-b-2 print:hidden mb-10">
                    <Link as="button" :href="route('dashboard.index')" class="btn btn-sm mb-5"><BaseIcon :path="mdiArrowLeft"/> Ke Halaman Utama</Link>
                ...
                <div class="grid grid-cols-5">
                    <span class="px-4 border-gray-800 border-l font-bold col-span-2">Pembelian Sawit</span>
                ...
                <div class="grid grid-cols-5" v-for="(item, index) in props.invoice.trades" :key="item.id">
                    ...
                    <span class="px-4 border-gray-800 border-l text-right">{{ Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.driver_fee) }}</span>
                    <span class="px-4 border-gray-800 border-x text-right">{{ Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.net_weight * item.driver_fee) }}</span>
                </div>
                <div class="grid grid-cols-5" >
                    <span class="px-4 pb-4 border-gray-800 border-l col-span-2"></span>
                    <span class="px-4 pb-4 border-gray-800 border-l"></span>
                    <span class="px-4 pb-4 border-gray-800 border-l"></span>
                    <span class="px-4 pb-4 border-gray-800 border-x text-right font-bold">{{ Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(trade.total) }}</span>
                </div>
                ...
                <div class="grid grid-cols-5 border-gray-800 border-b-2 font-bold">
...
onMounted( () => {
    ...
    createLine.value = 25 - (loanLine + tradeLine)

    let total = props.invoice.trades.reduce((arr, trades) => {
        arr.push(trades.net_weight * trades.driver_fee)
        return (arr)
    }, []);
